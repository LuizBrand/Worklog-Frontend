...
Directives and verification contracts for autonomous coding agents.
Treat hooks, tests, git state, screenshots, and evidence notes as the
real contract — most guidance below is advisory because the agent still
has to read and follow it.
...
- **Text** — type-check, lint, and tests pass. `stop-verify.sh` and
  `.githooks/pre-commit` enforce this when configured.
...
Verification commands are declared in `agent-md.toml [verify]`. When
unset, hooks fall back to heuristics (`tsconfig.json` → tsc,
`eslint.config.*` → eslint, lockfile → pnpm/yarn/npm).
...
This project runs **Next.js 16 + React 19**. APIs, conventions, and file
structure differ from training data. Read the relevant guide in
`node_modules/next/dist/docs/` before writing any code. Heed deprecation
notices.
<!-- END:nextjs-agent-rules -->

Full stack (versions, package manager, tooling) lives in
`memory/agents.md` — keep it accurate when dependencies change.
